...
import { ChevronLeft, SlidersHorizontal, Eye } from 'lucide-react';

export const TopViewed = () => {
  const navigate = useNavigate();
  const [isFilterOpen, setIsFilterOpen] = useState(false);
  const [filters, setFilters] = useState({ category: 'ALL', distanceKm: 50, sort: 'views' });

  const { data: result, isLoading } = useQuery({
    queryKey: ['topViewedAdsFeed'],
    ...
    <div className="space-y-4 pb-24 max-w-lg mx-auto px-1 sm:px-0">
      ...
      <div className="flex items-center justify-between pt-1">
        <div className="flex items-center gap-2">
          ...
          <h1 className="font-serif text-xl font-bold text-slate-900">Top Viewed</h1>
        </div>
      ...
      <div className="space-y-3.5 pt-2">
        {isLoading ? (
          ...
          <div className="p-8 text-center bg-white rounded-2xl border border-slate-100">
            <p className="font-serif text-slate-600 font-medium">No top viewed listings found.</p>
          </div>
        ) : (
          ads.map((ad) => (
            <div
            ...
            >
              ...
              <div className="flex-1 min-w-0 space-y-0.5">
                ...
                <div className="flex items-center gap-1 text-slate-400 text-[11px] font-light pt-0.5">
                  ...
                  <span>{formatCompactViews(ad.viewsCount || 12000)} views</span>
                </div>
              </div>
            </div>
          ))
